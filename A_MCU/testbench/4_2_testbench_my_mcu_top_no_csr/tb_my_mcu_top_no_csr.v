...
module tb_my_mcu_top_no_csr;
   ...
   task check_expected_mem;
      ...
      begin
         if (ext_mem[word_idx] !== expected_mem[word_idx]) begin
            ...
            $display("[FAIL] %-40s memory[0x%03h]=%h expected=%h",
                     case_name[expected_case_id[word_idx]], {word_idx, 2'b00},
                     ext_mem[word_idx], expected_mem[word_idx]);
         end else begin
            $display("[PASS] %-40s memory[0x%03h]=%h",
                     case_name[expected_case_id[word_idx]], {word_idx, 2'b00},
                     ext_mem[word_idx]);
         end
      end
   ...
   initial begin
      ...
      ext_mem[9'h1d0] = 32'h00000000;
      ext_mem[9'h1d1] = 32'h00000000;
      ext_mem[9'h1d2] = 32'h00000000;

      $display("------------------------------------------------------------");
      $display("my_mcu_top CSR-free smoke test");
      $display("  ALU/load/store/branch/jump over off-chip RF and memory");
      $display("------------------------------------------------------------");

      emit(enc_i(32'h600, 5'd0, FNC_ADD_SUB, 5'd1, OPC_ARI_ITYPE));
      emit(enc_i(0, 5'd1, FNC_LW, 5'd2, OPC_LOAD));
      store_result(5'd1, 32'h00000600, "ADDI base x1");
      store_result(5'd2, 32'hdeadbeef, "LW x2 from 0x600");

      emit(enc_u(32'h12345000, 5'd3, OPC_LUI));
      emit(enc_i(32'h678, 5'd3, FNC_ADD_SUB, 5'd3, OPC_ARI_ITYPE));
      store_result(5'd3, 32'h12345678, "LUI+ADDI x3");
      emit_nops(4);
      emit(enc_s(32'h740, 5'd3, 5'd0, FNC_SW));
      put_expected_case(32'h740, 32'h12345678, "SW x3 full");
      emit(enc_s(32'h744, 5'd3, 5'd0, FNC_SH));
      put_expected_case(32'h744, 32'h00005678, "SH x3 low half");
      emit(enc_s(32'h748, 5'd3, 5'd0, FNC_SB));
      put_expected_case(32'h748, 32'h00000078, "SB x3 byte0");

      emit(enc_i(5, 5'd0, FNC_ADD_SUB, 5'd4, OPC_ARI_ITYPE));
      emit(enc_i(3, 5'd0, FNC_ADD_SUB, 5'd5, OPC_ARI_ITYPE));
      emit(enc_r(FNC7_0, 5'd5, 5'd4, FNC_ADD_SUB, 5'd6));
      store_result(5'd6, 32'h00000008, "ADD x6");
      emit(enc_r(FNC7_0, 5'd5, 5'd4, FNC_SLL, 5'd7));
      store_result(5'd7, 32'h00000028, "SLL x7");
      emit(enc_r(FNC7_0, 5'd5, 5'd4, FNC_XOR, 5'd8));
      store_result(5'd8, 32'h00000006, "XOR x8");

      emit(enc_i(0, 5'd0, FNC_ADD_SUB, 5'd9, OPC_ARI_ITYPE));
      emit(enc_b(8, 5'd4, 5'd4, FNC_BEQ));
      emit(enc_i(1, 5'd0, FNC_ADD_SUB, 5'd9, OPC_ARI_ITYPE));
      emit(enc_i(2, 5'd0, FNC_ADD_SUB, 5'd9, OPC_ARI_ITYPE));
      store_result(5'd9, 32'h00000002, "BEQ taken");

      emit(enc_i(0, 5'd0, FNC_ADD_SUB, 5'd10, OPC_ARI_ITYPE));
      emit(enc_b(8, 5'd5, 5'd4, FNC_BNE));
      emit(enc_i(1, 5'd0, FNC_ADD_SUB, 5'd10, OPC_ARI_ITYPE));
      emit(enc_i(3, 5'd0, FNC_ADD_SUB, 5'd10, OPC_ARI_ITYPE));
      store_result(5'd10, 32'h00000003, "BNE taken");

      i = pc_word;
      emit(enc_j(8, 5'd11));
      emit(enc_i(1, 5'd0, FNC_ADD_SUB, 5'd12, OPC_ARI_ITYPE));
      emit(enc_i(9, 5'd0, FNC_ADD_SUB, 5'd13, OPC_ARI_ITYPE));
      store_result(5'd11, (i * 4) + 4, "JAL link");
      store_result(5'd12, 32'h00000000, "JAL skipped");
      store_result(5'd13, 32'h00000009, "JAL target");

      emit(enc_i((pc_word + 3) * 4, 5'd0, FNC_ADD_SUB, 5'd14, OPC_ARI_ITYPE));
      i = pc_word;
      emit(enc_i(0, 5'd14, FNC_ADD_SUB, 5'd15, OPC_JALR));
      emit(enc_i(1, 5'd0, FNC_ADD_SUB, 5'd16, OPC_ARI_ITYPE));
      emit(enc_i(10, 5'd0, FNC_ADD_SUB, 5'd17, OPC_ARI_ITYPE));
      store_result(5'd15, (i * 4) + 4, "JALR link");
      store_result(5'd16, 32'h00000000, "JALR skipped");
      store_result(5'd17, 32'h0000000a, "JALR target");
      ...
      $display("CSR-free smoke program words = %0d", pc_word);
      ...
      if (gpio_out !== 8'h00) begin
         error_cnt = error_cnt + 1;
         $display("[FAIL] gpio_out=%h expected=00", gpio_out);
      end else begin
      ...
      if (gpio_oe !== 8'h00) begin
         error_cnt = error_cnt + 1;
         $display("[FAIL] gpio_oe=%h expected=00", gpio_oe);
      end else begin
         $display("[PASS] gpio_oe = %h", gpio_oe);
      end

      if (error_cnt == 0)
         $display("[TB PASS] my_mcu_top CSR-free smoke test passed");
      else
         $fatal(1, "[TB FAIL] my_mcu_top CSR-free smoke mismatch. errors=%0d", error_cnt);
