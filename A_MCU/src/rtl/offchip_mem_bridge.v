...
module offchip_mem_bridge (
    ...
    reg clk_sys_sync;
    reg clk_sys_prev;
    wire clk_sys_rise = clk_sys_sync & !clk_sys_prev;
    ...
    wire [3:0]  next_sel  = dbus_req ? i_dbus_sel : 4'b1111;
    wire [31:0] next_addr = dbus_req ? i_dbus_adr : i_ibus_adr;
    wire [31:0] next_dat  = dbus_req ? i_dbus_dat : 32'h0000_0000;

    assign o_mem_sck = o_mem_sync ? ~i_clk_fast : 1'b0;
    assign o_mem_mosi = shift_tx[69];

    always @(posedge i_clk_fast) begin
        clk_sys_meta <= i_clk_sys;
        clk_sys_sync <= clk_sys_meta;
        clk_sys_prev <= clk_sys_sync;
                    ...
                    if (clk_sys_rise) begin
                        if (dbus_req || ibus_req) begin
                            if (req_pending) begin
                                ...
                                active_we <= next_we;
                                shift_tx <= {next_we, next_ibus, next_sel, next_addr, next_dat};
                                shift_rx <= 32'b0;
                                bit_count <= 7'd70;
                                o_mem_sync <= 1'b1;
                                req_pending <= 1'b0;
