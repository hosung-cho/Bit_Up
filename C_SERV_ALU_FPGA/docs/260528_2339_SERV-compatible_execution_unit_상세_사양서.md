...
ASIC
  ...
  - result latch
  - 4-bit command/response interface
```
...
`serv_exec_unit`은 RISC-V 명령어 전체를 직접 실행하지 않는다. FPGA가 instruction을 decode한 뒤 필요한 연산을 `cmd_op`로 변환해 ASIC에 보낸다. ASIC은 두 32-bit operand를 4-bit nibble serial로 수신하고, 내부에서는 SERV 방식에 맞춰 1-bit씩 32cycle 동안 연산한 뒤 결과를 다시 4-bit nibble serial로 반환한다.
...
| Signal | Direction | Width | Description |
| --- | --- | ---: | --- |
| `cmd_valid` | input | 1 | FPGA가 command nibble을 제공함 |
| `cmd_ready` | output | 1 | ASIC이 command nibble을 받을 수 있음 |
| `cmd_op` | input | 4 | operation code |
| `cmd_a` | input | 4 | operand A nibble |
| `cmd_b` | input | 4 | operand B nibble |
| `cmd_last` | input | 1 | 현재 nibble이 마지막 command nibble임 |

Command transfer는 `cmd_valid & cmd_ready`가 1인 clock edge에서 성립한다. Operand는 little-endian nibble order로 전송한다.
...
cycle 5: [23:20]
cycle 6: [27:24]
cycle 7: [31:28], cmd_last=1
```

정상 32-bit operation은 8개의 command nibble을 사용한다. `cmd_last`가 먼저 들어오면 그 시점까지 수신된 하위 nibble만 유효하고 나머지는 0으로 남는다. 현재 FPGA bridge는 항상 8개 nibble을 전송한다.

### 4.3 Response Channel

| Signal | Direction | Width | Description |
| --- | --- | ---: | --- |
| `rsp_valid` | output | 1 | ASIC이 result nibble을 제공함 |
| `rsp_ready` | input | 1 | FPGA가 result nibble을 받을 수 있음 |
| `rsp_result` | output | 4 | result nibble |
| `rsp_flags` | output | 6 | status/debug flags |
| `rsp_last` | output | 1 | 현재 nibble이 마지막 response nibble임 |

Response transfer는 `rsp_valid & rsp_ready`가 1인 clock edge에서 성립한다. Result도 little-endian nibble order로 반환한다.

```text
cycle 0: result[3:0]
cycle 1: result[7:4]
cycle 2: result[11:8]
cycle 3: result[15:12]
cycle 4: result[19:16]
cycle 5: result[23:20]
cycle 6: result[27:24]
cycle 7: result[31:28], rsp_last=1
```
...
`debug_state`는 bring-up과 FPGA 연동 디버깅용이다. 최종 pad budget이 빡빡한 chip-top에서는 생략하거나 scan/debug mux로 묶을 수 있다.
...
`rsp_flags`는 다음과 같이 생성된다.

```verilog
rsp_flags = {~cmp_r, cmp_r, is_cmp & cmp_r, is_cmp & ~cmp_r, is_sub, 1'b1};
```

| Bit | Meaning |
| ---: | --- |
| `[5]` | inverted final compare latch |
| `[4]` | final compare latch |
| `[3]` | compare operation이고 compare result가 true |
| `[2]` | compare operation이고 compare result가 false |
| `[1]` | SUB operation indicator |
| `[0]` | response valid marker, 항상 1 |

현재 flags는 architectural flag register가 아니라 FPGA bring-up과 protocol debug를 위한 보조 정보다. FPGA 시스템의 architectural decision은 기본적으로 `rsp_result`를 사용해야 한다.
...
| State | Encoding | Role |
| --- | ---: | --- |
| `ST_IDLE` | `3'd0` | idle, 첫 command nibble 수신 대기 |
| `ST_LOAD` | `3'd1` | operand nibble 수신 |
| `ST_EXEC` | `3'd2` | 32-cycle bit-serial execution |
| `ST_SEND` | `3'd3` | result nibble 송신 |

정상 operation 흐름:

```text
ST_IDLE
  -> first command nibble accepted
ST_LOAD
  -> remaining command nibbles accepted
ST_EXEC
  -> 32 cycles using original SERV ALU
ST_SEND
  -> 8 response nibbles sent
ST_IDLE
```
...
| response send | 8 |
| total command-first to response-last | 48 |

첫 command nibble이 handshake된 cycle을 0으로 보면, 마지막 command nibble은 cycle 7, execute는 cycle 8부터 39, 첫 response nibble은 cycle 40, 마지막 response nibble은 cycle 47에 handshake된다.
...
`serv_exec_fpga_bridge`는 FPGA 내부에서 쓰기 쉬운 32-bit request/response 인터페이스를 ASIC의 4-bit nibble protocol로 변환한다.
...
| `ST_SEND` | 8개 command nibble을 ASIC으로 전송 |
| `ST_RECV` | 8개 response nibble을 ASIC에서 수신 |
...
- 4-bit nibble command 분해
- ASIC command handshake
- ASIC response nibble 수신
- 32-bit response 재조립
- operation 결과 일치
...
```text
1. cmd_ready 확인
2. operand[3:0]부터 4-bit씩 8회 전송
3. 마지막 command nibble에서 cmd_last assert
4. rsp_valid 대기
5. result[3:0]부터 4-bit씩 8회 수신
...
- `rsp_flags`는 architectural state로 정의하지 않는다.
...
- command와 response는 little-endian nibble order다.
...
`serv_exec_unit`은 SERV 원본 `serv_alu`를 ASIC 내부에 포함한 32-bit bit-serial execution unit이다. FPGA가 시스템과 RISC-V control plane을 담당하고 ASIC은 ALU/compare execution plane을 담당한다. 현재 구현은 ADD/SUB/logic/compare subset을 지원하며, 4-bit nibble serial protocol로 operand와 result를 교환한다. qflow place-and-route 결과 1mm x 1mm 제약을 만족했고, Vivado RTL simulation도 unit/bridge 모두 통과했다.
